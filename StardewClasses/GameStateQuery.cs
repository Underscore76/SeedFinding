...
namespace StardewValley
{
    ...
    public class GameStateQuery
    {
        ...
        public static class Helpers
        {
            ...
            {
                Console.Write($"Failed parsing condition '{string.Join(" ", query)}': {reason}.", exception);
                return false;
            }
        ...
        public static class DefaultResolvers
        {
            ...
            public static bool DAYS_PLAYED(string[] query, GameStateQueryContext context)
            {
                ...
                uint daysPlayed = Game1.DaysPlayed;
            ...
            public static bool LOCATION_SEASON(string[] query, GameStateQueryContext context)
            {
                ...
                {
                    if (string.Equals(Game1.season.ToString(), query[i], StringComparison.OrdinalIgnoreCase))
                    {
                        return true;
                    }
                }
            ...
            public static bool WORLD_STATE_FIELD(string[] query, GameStateQueryContext context)
            {
                ...
                if (actualValue != null)
                {
                    if (!(actualValue is bool actual))
                    {
                        ...
                        {
                            if (actualValue is string actual3)
                            {
                                return string.Equals(actual3, expectedValue, StringComparison.OrdinalIgnoreCase);
                            }
                            return string.Equals(actualValue.ToString(), expectedValue, StringComparison.OrdinalIgnoreCase);
                        }
                        if (int.TryParse(expectedValue, out var minValue) && actual2 >= minValue)
                        {
        ...
        static GameStateQuery()
        {
            ...
            {
                GameStateQueryDelegate queryDelegate = (GameStateQueryDelegate)Delegate.CreateDelegate(typeof(GameStateQueryDelegate), method2);
                GameStateQuery.Register(method2.Name, queryDelegate);
            }
            array = methods;
            foreach (MethodInfo method in array)
            {
                ...
                if (attribute != null)
                {
                    ...
                    {
                        GameStateQuery.RegisterAlias(aliases[j], method.Name);
                    }
                }
            }
        }
